...
void main() {
    ...
    float diffuse = max(dot(-light_direction, normal), 0.0) * 0.4;

    float sun_strength = calculate_shadows(sun_position) * 0.4;

    outColor = (ambient_strength + sun_strength + diffuse) * texture(tex0, UV);
}
